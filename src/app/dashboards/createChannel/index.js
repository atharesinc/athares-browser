...
class CreateChannel extends Component {
  ...
  onSubmit = async (e) => {
    ...
    this.props.history.push(`/app/circle/${this.props.getActiveCircle.activeCircle.id}/channel/${res.data.createChannel.id}`);
  };
  render() {
    ...
    if (error) {
      return (
        <div
          ...
          className="pa2">
          <h1 className="mb3 mt0 lh-title mt4 f3 f2-ns">Error connecting to network</h1>
        </div>
      );
    }
    if (this.state.loading || loading) {
      return (
        <div
          ...
          className="pa2">
          <Loader />
          {this.state.loading && <h1 className="mb3 mt0 lh-title mt4 f3 f2-ns">Creating Channel</h1>}
        </div>
      );
    }
    return (
        ...
        <form className="pa4 white wrapper" onSubmit={this.onSubmit} id="create-circle-form">
          <Scrollbars style={{ height: '100%', width: '100%' }}>
            <article className="cf">
              ...
              <time className="f7 ttu tracked white-80">Create a new channel within {activeCircle.name}</time>
              <div className="fn mt4">
                <div className="measure mb4">
                  ...
                  />
                  <ErrorSwap
                    condition={!this.state.isTaken || this.props.checkIfNameUnique.Circle.channels.length !== 0}
                    normal={
                ...
                <div className="mv4">
                  <label htmlFor="comment" className="f6 b db mb2">
                    Description <span className="normal white-80">(optional)</span>
                  </label>
                  <textarea
                    id="comment"
                    name="comment"
            ...
            <div id="comment-desc" className="f6 white-80">
              By pressing "Create Channel" you will create a new government with a the above name and description. After this point, all changes must be made through the democratic
              revision process.
            </div>
