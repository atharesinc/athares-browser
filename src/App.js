...
class App extends Component {
  ...
  routeFix = () => {
    if (/\/login/.test(window.location.pathname)) {
      document.getElementById('root').removeEventListener('mousemove', this.parallaxApp, true);
      document.getElementById('root').addEventListener('mousemove', this.parallaxLogin, true);
      document.getElementById('root').style.overflow = 'hidden';
    } else if (/\/app/.test(window.location.pathname)) {
      document.getElementById('root').removeEventListener('mousemove', this.parallaxLogin, true);
        ...
        <PrivateRoute
          ...
          component={(props) => (
            <div className="wrapper high-img" id="main-layout">
              {this.state.width >= 768 ? <DesktopLayout {...props} /> : <MobileLayout {...props} />}
            </div>
          )}
        />
